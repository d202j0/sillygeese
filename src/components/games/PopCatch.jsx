...
const THEMES = {
  ...
  blue: {
    ...
    extraEffects: (area, cx, cy) => {
      ...
      sk.style.cssText = `position:absolute;pointer-events:none;font-size:28px;left:${cx-14}px;top:${cy+10}px;z-index:48;transition:opacity 0.6s ease-out,transform 0.6s ease-out;`
      sk.textContent = marks[Math.floor(Math.random() * marks.length)]
      area.appendChild(sk)
      // Animate with JS so we don't depend on external CSS keyframes
      requestAnimationFrame(() => {
        sk.style.opacity = '0'
        sk.style.transform = 'scale(0.5) rotate(20deg) translateY(30px)'
      })
      setTimeout(() => sk.remove(), 650)
    },
  },
}
