...
function useAudio() {
  ...
  const match = useCallback(() => {
    [523, 659, 784].forEach((f, i) => tone(f, 'triangle', 0.25, 0.25, i * 0.1))
  }, [tone])
  const noMatch = useCallback(() => tone(200, 'sawtooth', 0.15, 0.2), [tone])
  const win = useCallback(() => {
    [523,659,784,1047,1319,1568].forEach((f,i) => tone(f, 'triangle', 0.3, 0.28, i * 0.1))
  }, [tone])
...
export default function MemoryFlip() {
  const [themeKey, setThemeKey]     = useState(null)   // null = theme picker
  ...
  const [locked, setLocked]         = useState(false)  // block input during check
  const [winState, setWinState]     = useState(false)  // board cleared
  ...
  const buildGrid = useCallback((lvlIdx) => {
    ...
    const pool = [...CARD_POOL].sort(() => Math.random() - 0.5).slice(0, lvl.pairs)
  ...
  useEffect(() => {
    if (themeKey) startLevel(levelIdx)
      ...
      >
        {cards.map((card, idx) => {
          ...
          return (
            <div
              key={card.id}
              className={`${styles.cardWrap} ${isFaceUp ? styles.faceUp : ''} ${isMatched ? styles.matched : ''}`}
        ...
        <div className={styles.winOverlay}>
          <div className={styles.winBox}>
            ...
            <div className={styles.winBtns}>
              ...
              <button
                className={`${styles.winBtn} ${styles.winBtnSecondary}`}
                onClick={replay}
              >
                Play again
              </button>
